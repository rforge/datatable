...
%% \VignetteIndexEntry{FAQs - data.table}
...
\subsection{Why has the DT alias been removed?}
It hasn't been removed, yet. It is no longer encouraged or mentioned in the docs or examples. We now use and encourage
list() to be passed as the j.  list() is a primitive and is much faster than passing DT() or data.table() as the j, especially when
there are many groups.  Internally this was a non trivial change, but the speedups are very large.
...
\section{When I run 'by' and trace the computation, why do I get one extra group at the beginning?"}
Internally, the largest group is found, and the j expression run for that group. The result then determines how much
memory need be allocated for the result, in one allocation before the j expression is run for each group.
When you are returning data only, you will not notice this.
If you are using 'by' for the side-effects, for example in Sweave output, and you get one extra graph
at the beginning, then set byretn=0.
<<>>=
DT[,{cat("Trace:",as.character(x[1]),length(y),"\n");print(objects());NULL},by="x",byretn=0]
@
...
Several reasons.
\begin{itemize}
\item Only the V1 column is grouped, the other 19 are ignored because the j expression doesn't use them.
...
\item R is an in memory column store i.e. V1 is contiguous in RAM and page fetches from RAM into L2 are minimised.
\end{itemize}


\section{General questions about the package}

\subsection{Why is v1.3 missing from the CRAN archive ?}
This is correct. v1.2 was released to CRAN in Aug 2008. Development of v1.3 then continued on r-forge. Any user that made
contact with us was pointed to the r-forge version, as were users on r-help on several occassions. These users were very
helpful in testing major enhancements. When 1.3 was ready for CRAN, we decided to mark it v1.4, so that users of
various versions of 1.3 would know to upgrade.
From now on our hope is to have a more regular release cycle to CRAN.
...
\subsection{I think its great. How do I suggest it to other people ?}
Thank you. Please put your comments on \url{http://crantastic.org/}

\subsection{I think its not great. How do I warn others about my experience ?}
Please put your comments on \url{http://crantastic.org/}. Try to make it constructive so we have a chance to improve.
