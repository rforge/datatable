test.data.table = function()
{
    ...
    if (!identical(TESTDT[J(c("g","d","e","d"),c("b","g","a","f")),v,mult="last"], INT(7,NA,NA,4))) stop("Test 54 failed")  # this tests (d,g) ok even though there is an NA in last match in the roll.
    ...
    if (!"TESTDT" %in% tables(silent=TRUE)[,as.character(NAME)]) stop("Test 69 failed") # an old test (from when NAME was factor) but no harm in keeping it
